...
function Contact() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSubmitError(
        `Не удалось отправить сообщение. Попробуйте ещё раз или напишите напрямую на ${COMPANY.email}.`,
      );
    } finally {
        ...
        <div className="lg:col-span-7">
          {submitted ? (
            <div className="card-soft p-6 md:p-8 space-y-3">
              <p className="text-lg font-semibold text-brand">✓ Спасибо!</p>
              <p className="text-sm text-foreground/85 leading-relaxed">
                Ваша заявка отправлена. Я свяжусь в течение 24 часов.
              </p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="card-soft p-6 md:p-8 space-y-5" noValidate>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <Field label="Имя" required>
                  <Input required name="name" placeholder="Ваше имя" disabled={isSubmitting} />
                </Field>
                <Field label="Компания">
                  <Input name="company" placeholder="Название компании (опционально)" disabled={isSubmitting} />
                </Field>
              </div>
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <Field label="Телефон">
                  <Input name="phone" placeholder="опционально" disabled={isSubmitting} />
                </Field>
              </div>
              ...
              <label className="flex items-start gap-3 text-sm text-muted-foreground">
                <Checkbox id="dsgvo-ru" checked={agreed} onCheckedChange={(v) => setAgreed(Boolean(v))} disabled={isSubmitting} required />
                <span>
                  Я ознакомился(ась) с{" "}
