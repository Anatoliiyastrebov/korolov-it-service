...
function Contact() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!payload.name || !payload.email || !payload.topic || !payload.message) {
      setSubmitError("Будь ласка, заповніть обов'язкові поля.");
      return;
    }
    ...
    try {
      ...
      setSubmitError(
        `Не вдалося надіслати повідомлення. Спробуйте ще раз або напишіть напряму на ${COMPANY.email}.`,
      );
    } finally {
        ...
        <div className="lg:col-span-7">
          {submitted ? (
            <div className="card-soft p-6 md:p-8 space-y-3">
              <p className="text-lg font-semibold text-brand">✓ Дякуємо!</p>
              <p className="text-sm text-foreground/85 leading-relaxed">
                Вашу заявку надіслано. Я зв'яжуся протягом 24 годин.
              </p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="card-soft p-6 md:p-8 space-y-5" noValidate>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <Field label="Ім'я" required>
                  <Input required name="name" placeholder="Ваше ім'я" disabled={isSubmitting} />
                </Field>
                <Field label="Компанія">
                  <Input name="company" placeholder="Назва компанії (опціонально)" disabled={isSubmitting} />
                </Field>
              </div>
              <div className="grid sm:grid-cols-2 gap-4">
                <Field label="E-mail" required>
                  <Input required type="email" name="email" placeholder="your@email.de" disabled={isSubmitting} />
                </Field>
                <Field label="Телефон">
                  <Input name="phone" placeholder="опціонально" disabled={isSubmitting} />
                </Field>
              </div>
              ...
              <label className="flex items-start gap-3 text-sm text-muted-foreground">
                <Checkbox id="dsgvo-ua" checked={agreed} onCheckedChange={(v) => setAgreed(Boolean(v))} disabled={isSubmitting} required />
                <span>
              ...
              {submitError && (
                <div role="alert" className="rounded-lg border border-destructive/30 bg-destructive/5 p-4 text-sm text-foreground/85">
                  {submitError}
                </div>
              )}
            </form>
