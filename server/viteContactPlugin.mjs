// Vite-плагин: добавляет /api/contact в dev-сервер,
// чтобы форма работала локально без отдельного backend-процесса.
// В production используется server/index.mjs (Express).

// Загружаем .env в process.env для dev-режима (Vite сам этого не делает
// для серверной части — только для import.meta.env на клиенте).
