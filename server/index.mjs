// Production-сервер для IONOS Node-runtime.
// Раздаёт собранный фронтенд из ./dist и обрабатывает POST /api/contact
// через общий обработчик server/contactHandler.mjs.
//
// Запуск:  npm run build && npm run start
// Порт:    process.env.PORT (IONOS назначит сам), fallback 8080.
