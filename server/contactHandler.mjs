// Общий обработчик /api/contact для DE/RU/UA форм.
// Используется и в dev (Vite middleware), и в prod (Express).
// Транспорт: SMTP IONOS (smtp.ionos.de:465 SSL).
//
// Требуемые ENV:
//   SMTP_HOST       — обычно smtp.ionos.de
//   SMTP_PORT       — 465 (SSL) или 587 (STARTTLS)
//   SMTP_USER       — полный e-mail ящика IONOS (например, info@korolov-it-service.de)
//   SMTP_PASS       — пароль ящика IONOS
//   MAIL_FROM       — отправитель в письме (обычно тот же info@korolov-it-service.de)
//   MAIL_TO         — получатель уведомлений (тоже info@korolov-it-service.de)
//   MAIL_SUBJECT    — (опционально) тема письма
//   CONTACT_RATE_LIMIT_PER_MIN — (опционально) макс. писем на IP в минуту, по умолчанию 5
...
function getTransporter() {
  ...
  if (!host || !user || !pass) {
    throw new Error(
      "SMTP не сконфигурирован. Заполните SMTP_HOST, SMTP_USER, SMTP_PASS в .env",
    );
  }
